...
def fit_model(model, index, dataLoader):
    with tf.device('/gpu:'+gpu):
        ...
        model.validation_data = dataLoader.validationData
        model.fit(train_x, train_y, callbacks=[metrics])
        return model
...
class CINCFederatedDataLoader:
    ...
    def clientData(self, index):
        rand = random.randint(0, len(self.clientsData[index][0])-16)
        result = [self.clientsData[index][0][rand:rand+16], self.clientsData[index][1][rand:rand+16]]
        if not result[0].any():
            print(rand, rand+16)
        return result
...
class CombinedFederatedDataLoader:
    ...
    def __init__(self, mit_file_path, cinc_train_path, cinc_validation_path, clients_count):
        ...
        self.validationData = self.mitLoader.validationData + [self.cincLoader.validationData[0]]
...
# %%
for i in range(1000):
    if not combinedLoader.clientData(7)[0].any():
        print("A")
